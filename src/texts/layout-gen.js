...
function packLines(words, start, end, targetWidth, forcedEnds) {
  const lineEnds = [];
  let lineStart = start;
  let width = 0;
...
export function generateLayout(words, { linesPerColumn = null, charsPerLine = 44, columnStartWord = null, note }) {
  ...
  if (!columnStartWord) {
    const ends = packLines(words, 0, words.length - 1, charsPerLine, forced);
  ...
  for (let k = 0; k < starts.length; k++) {
    ...
    const ends = packLines(words, colStart, colEnd, colTarget, forced);
    ends.forEach((e) => lineEnds.add(e));
